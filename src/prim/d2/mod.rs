...
impl<A> VLine<A> {
    ...
    pub fn pos(&self) -> &Vec<A> {
        &self.pos
    }

    pub fn dir(&self) -> &Vec<A> {
        &self.dir
    }
}
...
impl<A : Clone> Clone for Vec<A> {
    fn clone(&self) -> Vec<A> {
        Vec::new(self.0.clone(), self.1.clone())
    }
}
...
impl<A : Add<Output = A>> Add for Vec<A> {
    ...
    fn add(self, other : Self) -> Vec<A> {
        Vec::new(self.0 + other.0, self.1 + other.1)
    }
}
...
impl<A : Mul<Output = A> + Clone> Mul<A> for Vec<A> {
    ...
    fn mul(self, other : A) -> Vec<A> {
        Vec::new(self.0 * other.clone(), self.1 * other)
    }
}
